...
c = csv.writer(open("parasites_survie.csv", "wb"))
c2= csv.writer(open("parasites_score.csv", "wb"))
...
class Particle():
    ...
    def etat(self):
        if self.state == "mort":
            my_particles.remove(self)
            self.speed = 0
            self.size = 5
            self.colour = (0, 0, 0)

        if self.state == "sain":
            sane_fertility_random = random.randint(0, 10000)
            sane_mortality_random = random.randint(0, 10000)
            if sane_mortality_random < sane_mortality:
                self.state == "mort"

            if sane_fertility_random < sane_fertility \
                and (len(my_particles) < population_max):
        ...
        if self.state == "malade":
            self.colour = (200, 0, 0)
            self.speed *= 0.999
            infected_mortality_random = random.randint(0, 10000)
            if infected_mortality_random < infected_mortality:
        ...
        if self.state == "mort_mais_contagieux":
            ...
            self.speed = 0
            self.colour = (0, 100, 0)
            chance_mort_contagieux = random.randint(0, 10000)
            if chance_mort_contagieux < dead_but_contagious_time :
                self.state = "mort"
...
for infected_mortality in infected_mortality_range_test:

    for infectiosity_value in infectiosity_value_range_test:
        duration = []
        cause = ""
        ...
        #infected_mortality = 200  # chance sur 10000

        #sane_mortality = 5 # par defaut
        #sane_fertility = 40 #par defaut

        pygame.font.init()
        screen = pygame.display.set_mode((width, height))
        pygame.display.set_caption('Parasites_sim - infectiosity = '+str(infectiosity_value)+ " - mortality = " + str(infected_mortality))
        ...
        while running and main_loop:
            ...
            for i, particle in enumerate(my_particles):
                ...
                for particle2 in my_particles[i + 1:]:
                    collide(particle, particle2, infectiosity_value)
                particle.display()
            ...
            if nb_de_cycles % 50 == 0:
                ...
                if choix_xls == 1 :
                    c2.writerow([infected_mortality,infectiosity_value, nb_de_cycles, nb_sain, nb_malade, nb_mort])

            if ((nb_sain == 0) or (nb_malade == 0) or (nb_de_cycles > 15000)) and (nb_de_cycles > 50) :
                #list_infectiosity.append(infectiosity_value)
                duration = nb_de_cycles
                if nb_sain == 0 :
                    cause = "mort de l'hote"
                if nb_malade == 0:
                    cause = "mort du parasite"
                if choix_xls == 1 :
                    c.writerow ([infected_mortality,infectiosity_value,duration,cause])
                print str(infected_mortality) +", "+ str(infectiosity_value) + ", " + str(duration)+ ", " + cause
                running = False
